...
def total(line_number):
	...
	#Builds loop to remove lines from stellar spectra
	count_lines = 0
	while count_lines < line_number:
		random_index = random.randint(0, len(lande_g1) - 1)
		lande_g1 = np.delete(lande_g1, [random_index])
		red1 = np.delete(red1, [random_index])
		blue1 = np.delete(blue1, [random_index])
		weight1 = np.delete(weight1, [random_index])
		count_lines += 1

	return np.array(lande_g1), np.array(red1), np.array(blue1), np.array(weight1)
...
x = []
for xval in range(0,2000,40):
	x.append(xval)

y = []
for yval in range(40,2040,40):
	y.append(yval)
...
for number in range(0,500,5):
	global lande_g
	global red
	global blue
	global weight
	lande_g, red, blue, weight = total(number)
	...
	time_per_section.append(section_time)
	print "Time: " + str(time_per_section)

print std_hist_array
print mean_array
...
p_std_hist = np.savetxt("/home/katie/files/rand_std_data_hist.txt", std_hist_array)
p_mad = np.savetxt("/home/katie/files/rand_mad_data.txt", mad_array)
p_std_MCavg = np.savetxt("/home/katie/files/rand_std_data_MCavg.txt", std_MCavg_array)
p_std_RV = np.savetxt("/home/katie/files/rand_RV_data.txt", mean_array)
p_amp = np.savetxt("/home/katie/files/rand_amp_data.txt", amp_array)
p_time = np.savetxt("/home/katie/files/rand_time_data.txt", time_per_section)
