...
def total(line_number):
	...
	while count_lines < line_number:
		reddest_index = np.argmax(red1)
		lande_g1 = np.delete(lande_g1, [reddest_index])
		red1 = np.delete(red1, [reddest_index])
		blue1 = np.delete(blue1, [reddest_index])
		weight1 = np.delete(weight1, [reddest_index])
		count_lines += 1
...
x = []
for xval in range(0,8,4):
	x.append(xval)

y = []
for yval in range(4,12,4):
	y.append(yval)
