...
def section(start, end):
	...
	return std_hist_array, std_MCavg_array, mad_array, mean_array, amp_array, time_per_section

#Splits job up into specific sections, to be fastest must have more sections than # of CPUs
x = []
for xval in range(0,10,5):
	x.append(xval)

y = []
for yval in range(5,15,5):
	y.append(yval)
...
for array in np.array(results):
	...
	final_time = np.append(final_time, np.array(array[5]))
...
"""
p_std_hist = np.savetxt("/home/katie/files/3long_p_std_data_hist.txt", final_hist)
p_mad = np.savetxt("/home/katie/files/3long_p_mad_data.txt", final_mad)
p_std_MCavg = np.savetxt("/home/katie/files/3long_p_std_data_MCavg.txt", final_MCavg)
p_std_RV = np.savetxt("/home/katie/files/3long_p_RV_data.txt", final_mean)
p_amp = np.savetxt("/home/katie/files/3long_p_amp_data.txt", final_amp)
p_time = np.savetxt("/home/katie/files/3long_p_time_data.txt", final_time)
"""
